...
// For LinuxToWindows - add _alloca() for stack dynamic allocation
		     ...
		     struct iw_point *wrqu, char *extra)
{
	...
	char *input = _alloca(wrqu->length + 1);
		    ...
		    struct iw_point *wrqu, char *extra)
{
	...
	int ret;
	PADAPTER padapter = rtw_netdev_priv(dev);
	char *input = _alloca(wrqu->length);
		...
		struct iw_point *wrqu, char *extra)
{
	...
	u8		*input = _alloca(wrqu->length + 1);
	PADAPTER padapter = rtw_netdev_priv(dev);
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	u8 *input = _alloca(wrqu->data.length + 1);
	u32 bt_coexist;
...
{
	PADAPTER padapter = rtw_netdev_priv(dev);
	char	*input = _alloca(wrqu->data.length);
	int	qAutoLoad = 1;
		...
		union iwreq_data *wrqu, char *extra)
{
	...
	u8		*input = _alloca(wrqu->data.length);
